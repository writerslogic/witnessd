...
fn test_full_pipeline() {
    ...
    // 1. Initialize
    let fingerprint = api::init_witnessd(Some(data_dir.to_string_lossy().to_string()), None).unwrap();
    ...
    let report = reports.iter().find(|r| r.file_path.contains("test.txt")).expect("Test file not in reports");
