...
func TestMobileEngine(t *testing.T) {
	mobile := NewMobileEngine()

	if err := mobile.StartSession("com.test.mobile", "doc1", "mobile test"); err != nil {
		t.Fatal(err)
	}

	if !mobile.HasActiveSession() {
		t.Error("Expected active session")
	}

	// Simulate typing
	chars := []int32{'h', 'e', 'l', 'l', 'o'}
	for _, c := range chars {
		delay, err := mobile.OnKeyDown(c)
		if err != nil {
			t.Errorf("OnKeyDown failed: %v", err)
		}
		_ = delay

		if err := mobile.OnTextCommit(string(rune(c))); err != nil {
			t.Errorf("OnTextCommit failed: %v", err)
		}
	}

	count := mobile.GetSampleCount()
	t.Logf("Mobile: collected %d samples", count)

	// End session
	_, err := mobile.EndSession()
	if err != nil {
		t.Fatalf("EndSession failed: %v", err)
	}
}

func TestZoneFromPosition(t *testing.T) {
	// Test mobile position-based zone detection
	testCases := []struct {
		x, y         float32
		expectedZone int
	}{
		{0.05, 0.5, 0}, // Far left = pinky
		{0.15, 0.5, 1}, // Left ring
		{0.25, 0.5, 2}, // Left middle
		{0.35, 0.5, 3}, // Left index
		{0.45, 0.5, 3}, // Left index (extended)
		{0.55, 0.5, 4}, // Right index
		{0.65, 0.5, 4}, // Right index (extended)
		{0.75, 0.5, 5}, // Right middle
		{0.85, 0.5, 6}, // Right ring
		{0.95, 0.5, 7}, // Right pinky
	}

	for _, tc := range testCases {
		zone := ZoneFromPosition(tc.x, tc.y)
		if zone != tc.expectedZone {
			t.Errorf("Position (%.2f, %.2f): expected zone %d, got %d",
				tc.x, tc.y, tc.expectedZone, zone)
		}
	}
}
...
func TestMobileEngineFullAPI(t *testing.T) {
	mobile := NewMobileEngine()

	// Test all mobile API methods
	if err := mobile.StartSession("com.test.mobile", "doc1", "test"); err != nil {
		t.Fatal(err)
	}

	// OnKeyDownWithZone
	delay, err := mobile.OnKeyDownWithZone(0, 'q')
	if err != nil {
		t.Errorf("OnKeyDownWithZone failed: %v", err)
	}
	mobile.OnTextCommit("q")
	_ = delay

	// OnKeyDownWithPosition
	delay, err = mobile.OnKeyDownWithPosition(0.75, 0.5, 'i')
	if err != nil {
		t.Errorf("OnKeyDownWithPosition failed: %v", err)
	}
	mobile.OnTextCommit("i")
	_ = delay

	// Test delete
	if err := mobile.OnTextDelete(1); err != nil {
		t.Errorf("OnTextDelete failed: %v", err)
	}

	// GetSessionInfo
	info := mobile.GetSessionInfo()
	if info == "" {
		t.Error("Expected non-empty session info")
	}

	// GetProfile
	profile := mobile.GetProfile()
	if profile == "" {
		t.Error("Expected non-empty profile")
	}

	// EndSessionFull
	evidence, err := mobile.EndSessionFull()
	if err != nil {
		t.Fatalf("EndSessionFull failed: %v", err)
	}
	if evidence == "" {
