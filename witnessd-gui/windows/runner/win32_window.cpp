...
bool Win32Window::Create(const std::wstring& title,
  ...
  int window_w = Scale(size.width, scale_factor);
  int window_h = Scale(size.height, scale_factor);
  ...
  if (origin.x == 0 && origin.y == 0) {
    ...
    if (GetMonitorInfo(monitor, &monitor_info)) {
      ...
      window_x = work.left + ((work.right - work.left) - window_w) / 2;
      window_y = work.top + ((work.bottom - work.top) - window_h) / 2;
    }
  }
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    case WM_KEYDOWN: {
      ...
      if (ctrl_down && child_content_ != nullptr) {
        switch (wparam) {
          case 'R':
          case 'E':
          case 'L':
    ...
    case WM_GETMINMAXINFO: {
      if (minimum_size_.width > 0 && minimum_size_.height > 0) {
        ...
        info->ptMinTrackSize.x =
            static_cast<LONG>(Scale(minimum_size_.width, scale_factor));
        info->ptMinTrackSize.y =
            static_cast<LONG>(Scale(minimum_size_.height, scale_factor));
        return 0;
      }
      break;
    }
...
void Win32Window::SetAppId(const std::wstring& app_id) {
...
void Win32Window::UpdateTheme(HWND const window) {
  ...
  if (result == ERROR_SUCCESS) {
    ...
    if (enable_dark_mode) {
      const COLORREF caption_color = RGB(14, 17, 22);
      const COLORREF text_color = RGB(236, 239, 244);
      const COLORREF border_color = RGB(23, 27, 34);
